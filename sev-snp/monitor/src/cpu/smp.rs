...
pub fn smp_prepare_bios_vmpl(caa_pa: PhysAddr) -> bool {
    ...
    //
    //GUESTOS
    let ret: u32 = rmpadjust(caa.va().as_u64(), RMP_4K, VMPL_RWX | VMPL::Vmpl3 as u64);
    if ret != 0 {
        return false;
    }

    //GUESTOS
    let ret: u32 = rmpadjust(vmsa.va().as_u64(), RMP_4K, VMPL_R | VMPL::Vmpl3 as u64);
    if ret != 0 {
        return false;
    }
    ...
    //GUESTOS
    let ret: u32 = rmpadjust(vmsa.va().as_u64(), RMP_4K, VMPL_VMSA | VMPL::Vmpl3 as u64);
    if ret != 0 {
        return false;
    }
