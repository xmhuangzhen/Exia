...
use crate::protocols::initialize_logframes;
use crate::protocols::protect_log_entry;
use crate::protocols::dump_logs;
use crate::protocols::create_enclave_test;
use crate::protocols::init_enclave_input_buffer;
use crate::protocols::transfer_input_to_enclave;
use crate::protocols::verify_buffer_mapping;
// use crate::protocols::*;
...
/// 14 
pub const SVSM_ENCLAVE_SERVICE_CREATE: u32 = 14;
/// 15
pub const SVSM_INIT_ENCLAVE_INPUT_BUFFER: u32 = 15;
/// 16
pub const SVSM_TRANSFER_INPUT_TO_ENCLAVE: u32 = 16;
/// 17
pub const SVSM_VERIFY_BUFFER_MAPPING: u32 = 17;
...
unsafe fn demote_vmsa_page(gpa: PhysAddr) -> Result<(), u64> {
    ...
    //GUESTOS
    grant_vmpl_access(map.va(), RMP_4K, VMPL::Vmpl3 as u8);
    Ok(())
}
...
unsafe fn __handle_vcpu_create_request(
    ...
    //GUESTOS    
    // Only VMPL3 is currently supported
...
unsafe fn handle_pvalidate(vmsa: *mut Vmsa, entry: *const PvalidateEntry) -> (bool, bool) {
    ...
    //GUESTOS
        let ret: u32 = grant_vmpl_access(map.va(), page_size, VMPL::Vmpl3 as u8);
        if ret != 0 {
            (*vmsa).set_rax(SVSM_ERR_PROTOCOL_BASE + ret as u64);
            return (false, flush);
        }
...
fn is_in_calling_area(gpa: PhysAddr) -> bool {
    let gfn: PhysFrame = PhysFrame::containing_address(gpa);
    //GUESTOS
    let caa_gpa: PhysAddr = unsafe { PERCPU.caa(VMPL::Vmpl3) };
...
unsafe fn handle_empty_request(vmsa: *mut Vmsa) {
    (*vmsa).set_rax(SVSM_SUCCESS);
}
...
pub unsafe fn core_handle_request(callid: u32, vmsa: *mut Vmsa) {
    match callid {
        ...
        // our
        SVSM_INIT_ENCLAVE_INPUT_BUFFER => init_enclave_input_buffer(vmsa),
        // SVSM_INIT_ENCLAVE_INPUT_BUFFER => handle_empty_request(vmsa),
        SVSM_TRANSFER_INPUT_TO_ENCLAVE => transfer_input_to_enclave(vmsa),
        // SVSM_TRANSFER_INPUT_TO_ENCLAVE => handle_empty_request(vmsa),
        SVSM_VERIFY_BUFFER_MAPPING => verify_buffer_mapping(vmsa),
        // SVSM_VERIFY_BUFFER_MAPPING => handle_empty_request(vmsa),
